...
class Upsample(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale_factor, mode):
        ...
        self.upsample = nn.Upsample(scale_factor=scale_factor, mode='nearest', align_corners=True)
...
class AuNet(nn.Module):
    def __init__(self):
        ...
        # self.pass1_5 = BasicBlock(128, 64, kernel_size=3, stride=1)
        ...
        self.my_pass_1 = BasicBlock(192, 64, kernel_size=3, stride=1)
        self.my_pass_2 = nn.Conv2d(192, 3, kernel_size=3, stride=1, padding=1)
        self.my_pass_3 = nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1)
        self.my_pass_4 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.my_pass_5 = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
    ...
    def forward(self, x):
        ...
        # map7 = torch.cat((self.pass1_4(map4), self.upsample1_2(map5)), 1)

        m4 = self.my_pass_3(map4)
        m5 = self.my_pass_3(map5)
        m6 = self.my_pass_3(map6)
        m7 = self.my_pass_2(map7)
        # map8 = torch.cat((self.my_pass_3(map7), self.my_pass_4(map5), self.my_pass_5(map6)), 1)
        # out=self.my_pass_1(map7)
        # out=self.my_pass_2(out)
        out = self.pass1_5(map7)
        out = self.pass1_6(out)
        out = self.pass1_7(out)
        out = self.pass1_8(out)

        return out, (m4, m5, m6, m7)
